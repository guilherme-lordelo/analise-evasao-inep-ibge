...
import pandas as pd
from brpipe.inep.config import ANOS, VARIAVEIS_YAML
...
def agrega_quantitativas(df: pd.DataFrame, nivel: str = "municipal") -> pd.DataFrame:
    ...
    if nivel == "municipal":
        colunas_groupby = VARIAVEIS_YAML.campos_padrao

    elif nivel == "estadual":
        colunas_groupby = [VARIAVEIS_YAML.campos_padrao[1]]
...
def agrega_categoricas_ano(df: pd.DataFrame, ano: str, nivel: str = "municipal") -> pd.DataFrame:
    ...
    col_peso = f"{VARIAVEIS_YAML.coluna_peso}_{ano}"
...
def agrega_categoricas(
    ...
    for ano, leitor in leitores_por_ano.items():
        ...
        if result_mun is None:
            ...
            # MUNICIPAL
            chave_municipal = VARIAVEIS_YAML.campos_padrao
...
def merge_quantitativas_com_categoricas(
    ...
    result_mun = quant_mun.merge(
        cat_mun,
        on=VARIAVEIS_YAML.campos_padrao,
        how="left",
    )
