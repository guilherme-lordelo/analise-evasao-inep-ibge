...
def _transformar_sheet(tabela, sheet):
	...
	df = limpar(df)
	df = aplicar_schema_ibge(df, COLUNAS_BASE_IBGE, sheet.colunas_especificas)
	df = aplicar_merges_colunas(df, sheet)
	df = aplicar_transformacoes_colunas(df, sheet)
	df = remover_colunas(df, sheet)

	path_out = PROCESSED_IBGE / sheet.arquivo
	write_csv(df, path_out)


def transformar_ibge():
	iterar_sheets_ibge(TABELAS_IBGE, _transformar_sheet)
